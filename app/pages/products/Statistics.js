...
import { getDataGenaral, getDetailCurentBalance } from '@/app/services/statistics';
...
const Statistics = () => {
  ...
  const fetchDataGenaral = async () => {
    try {
      ...
      if (res && res.data && res.data.balance !== undefined && res.data.totalAmount !== undefined) {
  ...
  const columns = [
    ...
    { title: "Chủ tài khoản", dataIndex: "note", key: "note" },
    {
      title: "Số dư",
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', alignItems: "end" }}>
      <div style={statBoxStyle} className="flex justify-center flex-col items-center">
        <h3 className="font-bold text-lg">Lợi Nhuận</h3>
        <h1 className="font-bold text-3xl">
          {dataTotalAmount !== undefined ? new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(dataTotalAmount) : "0 ₫"}
        </h1>

      </div>
      <div style={statBoxStyles} className="flex justify-center flex-col items-center relative">
        <h3 className="font-bold text-lg">Số Dư Hiện Tại</h3>
        <h1 className="font-bold text-3xl">
          {databalance !== undefined ? new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(databalance) : "0 ₫"}
        </h1>

        <div className="absolute right-2 top-2"><ExclamationCircleOutlined onClick={showModal} /></div>
        <BaseModal title="Chi tiết số dư hiện tại" open={isModalOpen} onOk={handleOk} onCancel={handleCancel} offPadding>
